...
--// ~800 lines, self-contained, no external UI dependencies
...
---------------------------------------------------------------------------
local function shimFunction(name, fallback)
    if not getfenv()[name] and not _G[name] then
        getfenv()[name] = fallback or function() end
    end
end
...
if not getcustomasset then
    getcustomasset = function(path) return "rbxasset://fonts/families/GothamSSm.json" end
end
...
local CurrentGame = GameRegistry[CurrentPlaceId]
...
---------------------------------------------------------------------------
local function Main()
    -- Refs that persist across phases
    local BlurEffect
    local SplashGui
    local HubGui
    local NotifGui
    ...
    local function Notify(title, desc, duration, color)
        ...
        local titleSize = game:GetService("TextService"):GetTextSize(
            title, 14, Enum.Font.GothamBold, Vector2.new(maxW - 28, 1000)
        )
        local descSize = game:GetService("TextService"):GetTextSize(
            desc, 13, Enum.Font.GothamMedium, Vector2.new(maxW - 28, 1000)
        )

        local frameW = math.min(maxW, math.max(titleSize.X, descSize.X) + 32)
        local frameH = titleSize.Y + descSize.Y + 36
        ...
        local titleLabel = CreateInstance("TextLabel", {
            ...
            Size = UDim2.new(1, -20, 0, titleSize.Y),
            BackgroundTransparency = 1,
            FontFace = CustomFont,
            Text = title,
        ...
        local descLabel = CreateInstance("TextLabel", {
            ...
            Size = UDim2.new(1, -20, 0, descSize.Y),
            BackgroundTransparency = 1,
    ...
    local function CreateHub()
        ...
        local detailText = string.format(
            "ID: %d  •  Executor: %s  •  Games: %d",
            CurrentPlaceId, ExecutorName, SupportedGameCount
        )
        ...
        local execBtn = CreateInstance("TextButton", {
            ...
            BackgroundColor3 = CurrentGame and Theme.Accent or Color3.fromRGB(60, 58, 65),
            FontFace = CustomFont,
            Text = CurrentGame and "Execute Script" or "Not Supported",
            TextColor3 = Theme.Text,
            TextSize = 16,
            BorderSizePixel = 0,
            AutoButtonColor = false,
        ...
        local websiteBtn = CreateInstance("TextButton", {
            Position = UDim2.new(1, -halfW, 0, 0),
        ...
        local function CreateToggleButton()
            ...
            ToggleButton.MouseButton1Click:Connect(function()
                ...
                -- Simple: if drag distance is minimal, treat as click
        ...
        local function AddHover(btn, normalSize)
            ...
            btn.MouseEnter:Connect(function()
                if btn and btn.Parent then
                    QuickTween(btn, { Size = hoverSize }, 0.15)
                end
            end)
            btn.MouseLeave:Connect(function()
                if btn and btn.Parent then
                    QuickTween(btn, { Size = normalSize }, 0.15)
                end
            end)
        end
        ...
        -- Execute
        local execNormalSize = execBtn.Size
    ...
    UserInputService.InputBegan:Connect(function(input, processed)
        ...
        if input.KeyCode == Enum.KeyCode.RightControl or input.KeyCode == Enum.KeyCode.F9 then
            ...
            elseif HubOpen then
                -- Minimize
                -- We need a reference to the minimize function; 
                -- since hub creates its own, we trigger via the panel
                -- Simplest: just fire the min button
                -- But we can also just set a flag and recreate
                -- Cleanest approach: destroy and show toggle
                if HubGui and HubGui.Parent then
                    HubGui:Destroy(); HubGui = nil
                end
                pcall(function()
                ...
                if not ToggleGui or not ToggleGui.Parent then
                    -- Inline toggle creation for keyboard shortcut
                    ToggleGui = CreateInstance("ScreenGui", {
                        Name = "SolixToggle",
                        DisplayOrder = 1001,
                        ResetOnSpawn = false,
                    ...
                    -- Toggle draggable
                    local tDragging2 = false
                    local tDragStart2, tStartPos2

                    ToggleButton.InputBegan:Connect(function(input2)
                        if input2.UserInputType == Enum.UserInputType.MouseButton1
                            or input2.UserInputType == Enum.UserInputType.Touch then
                            tDragging2 = true
                            tDragStart2 = input2.Position
                            tStartPos2 = ToggleButton.Position
                    ...
                    UserInputService.InputChanged:Connect(function(input2)
                        if tDragging2 and (
                            input2.UserInputType == Enum.UserInputType.MouseMovement
                            or input2.UserInputType == Enum.UserInputType.Touch
                        ) then
                            local delta = input2.Position - tDragStart2
                            ToggleButton.Position = UDim2.new(
                                tStartPos2.X.Scale, tStartPos2.X.Offset + delta.X,
                                tStartPos2.Y.Scale, tStartPos2.Y.Offset + delta.Y
                            )
                        end
                    end)
                ...
                -- Not open, not minimized — open fresh
                CreateHub()
            end
        end
    end)
