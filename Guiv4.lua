--[[
    ╔═══════════════════════════════════════════════════════════════════╗
    ║  Solix Hub v4.1.0 — Production Loader (Keyless Edition)         ║
    ║  Premium Game Hub · Polished · Bug-Free · Production-Ready      ║
...
---------------------------------------------------------------------------
-- §6 ▸ Game Registry (keyed by GameId)
---------------------------------------------------------------------------
local GameRegistry = {
...
for _ in pairs(GameRegistry) do SupportedGameCount += 1 end
...
-- 8a: Connection Manager (prevents leaks across open/close cycles)
...
-- 8c: Tween wrapper (safe against destroyed instances)
...
-- 8f: Ripple effect on buttons
local function CreateRipple(button, inputPos)
    if not IsAlive(button) then return end
    local absPos  = button.AbsolutePosition
    local absSize = button.AbsoluteSize
    local rx = inputPos and (inputPos.X - absPos.X) or (absSize.X / 2)
    local ry = inputPos and (inputPos.Y - absPos.Y) or (absSize.Y / 2)
    ...
    }, 0.55, Enum.EasingStyle.Exponential)

    task.delay(0.6, function()
        if IsAlive(ripple) then ripple:Destroy() end
    end)
end

-- 8g: Draggable system (leak-free via connMgr)
local function MakeDraggable(handle, target, connMgr, onDrag)
    ...
    connMgr:Connect(UserInputService.InputChanged, function(input)
        ...
        if onDrag then onDrag(delta) end
    end)
    ...
    pcall(function()
        if isfile(Config.SettingsFile) then
            ...
            if type(data.LastPosX) == "number" and type(data.LastPosY) == "number" then
                ...
                -- Clamp to visible screen (leave at least 100px visible)
...
pcall(function()
    ...
    if not isfile(fontPath) then
        local data = game:HttpGet(
            "https://github.com/sametexe001/luas/raw/refs/heads/main/fonts/InterSemibold.ttf"
        )
    ...
    if isfile(fontPath) then
        local fontData = {
            name  = "InterSemiBold",
            faces = {{ name = "InterSemiBold", weight = 400, style = "Regular",
...
local function Main()
    local BlurEffect
    local SplashGui, HubGui, NotifGui, ToggleGui
    local ToggleButton

    -- Per-phase connection managers (disconnected on cleanup)
    ...
    local function Notify(title, desc, duration, color)
        ...
        local icon = NotifIcons[color] or "ℹ"
        ...
        local frameW   = math.clamp(math.max(titleSz.X, descSz.X) + 42, 180, maxW)
        ...
        local titleLabel = Create("TextLabel", {
            ...
            Size               = UDim2.new(1, -22, 0, titleSz.Y),
            BackgroundTransparency = 1,
            FontFace           = FontBold,
            Text               = titleTxt,
        ...
        local descLabel = Create("TextLabel", {
            Position           = UDim2.new(0, 14, 0, 10 + titleSz.Y + 4),
            Size               = UDim2.new(1, -22, 0, descSz.Y),
            BackgroundTransparency = 1,
            FontFace           = FontMedium,
            Text               = desc,
    ...
    local function LoadGameScript()
        ...
        if CurrentGame.ScriptURL and CurrentGame.ScriptURL ~= "" then
            local ok, err = pcall(function()
                ...
                assert(code and #code > 0, "Empty response from ScriptURL")
                local fn = loadstring(code)
                assert(fn, "loadstring compilation failed")
                fn()
            end)
        ...
        if CurrentGame.LuarmorId and CurrentGame.LuarmorId ~= "" then
            ...
            if ok then ScriptLoaded = true; return true end
            Notify("Load Error", "Luarmor failed: " .. tostring(err), 6, Theme.Error)
            return false
        end

        Notify("Error", "No loading method configured for this game.", 5, Theme.Error)
        return false
    end
            ...
            if t then
                t.Completed:Once(function()
                    if BlurEffect and BlurEffect.Parent then
                        BlurEffect:Destroy(); BlurEffect = nil
                    end
                end)
            end
        end)
    ...
    local function CreateSplash(onComplete)
        ...
        local backdrop = Create("Frame", {
            Size                 = UDim2.new(1, 0, 1, 0),
            BackgroundColor3     = Color3.fromRGB(6, 5, 9),
            BackgroundTransparency = 1,
            BorderSizePixel      = 0,
        }, SplashGui)
        ...
        task.spawn(function()
            ...
            -- Animated gradient sweep on title
            task.spawn(function()
            ...
            local stages = {
                ...
                { pct = 0.80, text = CurrentGame
                    and ("Found: " .. CurrentGame.Name)
                    or  "Game not in registry" },
                { pct = 1.00, text = "Ready" },
            }
    ...
    -- 10e ▸ TOGGLE BUTTON
    -------------------------------------------------------------------
    local CreateHub  -- forward declaration
    ...
    local function CreateToggleButton()
        ...
        task.spawn(function()
            while IsAlive(ToggleButton) and CurrentState == State.MINIMIZED do
                ...
                task.wait(1.5)
                if not IsAlive(ToggleButton) then break end
        ...
        toggleConns:Connect(ToggleButton.MouseEnter, function()
            if IsAlive(ToggleButton) then
                QuickTween(ToggleButton, { BackgroundTransparency = 0 }, 0.15)
            end
        end)
    end
    ...
    -------------------------------------------------------------------
    CreateHub = function()
        -- Guard: reject if already opening or closing
        ...
        -- Resolve position (saved or centered)
        ...
        -- FIX: Shadow matches panel anchor & position (offset +5 Y)
        local shadowFrame = Create("Frame", {
            Name             = "Shadow",
            AnchorPoint      = restoreAnchor,
            Position         = UDim2.new(
                restorePos.X.Scale, restorePos.X.Offset,
                restorePos.Y.Scale, restorePos.Y.Offset + 5
            ),
            Size             = UDim2.new(0, 0, 0, 0),
        ...
        local nameWidth = TextService:GetTextSize(
            Config.Name, 19, Enum.Font.GothamBold, Vector2.new(300, 50)
        ).X
        local versionBadge = Create("TextLabel", {
            AnchorPoint          = Vector2.new(0, 0.5),
            Position             = UDim2.new(0, 18 + nameWidth + 10, 0.5, 0),
        ...
        local infoCard = Create("Frame", {
            AnchorPoint          = Vector2.new(0.5, 0),
            Position             = UDim2.new(0.5, 0, 0, 62),
            Size                 = UDim2.new(0, panelW - 36, 0, 90),
            BackgroundColor3     = Theme.Surface,
            BorderSizePixel      = 0,
            BackgroundTransparency = 1,
            ZIndex               = 5,
        ...
        local statusDot = Create("Frame", {
            ...
            BackgroundColor3     = CurrentGame and Theme.Success or Theme.Error,
            BorderSizePixel      = 0,
            BackgroundTransparency = 1,
            ZIndex               = 6,
        }, infoCard)
        ...
        local execBtn = Create("TextButton", {
            ...
            Size                 = UDim2.new(0, panelW - 36, 0, 48),
        ...
        local execStroke = Create("UIStroke", {
            Color           = Theme.Border,
            Thickness       = 1,
            Transparency    = 1,
            ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
        ...
        local socialRow = Create("Frame", {
            ...
            Size                 = UDim2.new(0, panelW - 36, 0, 40),
            BackgroundTransparency = 1,
            ZIndex               = 5,
        }, panel)
        ...
        local footer = Create("TextLabel", {
            ...
            BackgroundTransparency = 1,
            FontFace             = FontMedium,
            Text                 = Config.Version .. "  ·  Keyless  ·  " .. SupportedGameCount .. " Games Supported",
            TextColor3           = Theme.TextTertiary,
            TextSize             = 10,
            TextTransparency     = 1,
            ZIndex               = 5,
        ...
        -- ═══════════════════════════════════════════
        -- DRAG (panel header → moves panel + shadow)
        -- ═══════════════════════════════════════════
        ...
        local function AnimateOpen()
            ...
            QuickTween(shadowFrame, {
                Size = UDim2.new(0, panelW + 24, 0, panelH + 24),
            ...
            for _, s in ipairs(allStrokes) do
                QuickTween(s, { Transparency = 0 }, 0.35)
            end

            -- Staggered element reveal
            for i, e in ipairs(fadeElements) do
                task.delay(i * 0.018, function()
                    if IsAlive(e.obj) then
                        QuickTween(e.obj, { [e.prop] = e.to }, 0.35, Enum.EasingStyle.Exponential)
                    end
                end)
            end
            ...
            --[[
                FIX: Set state to HUB_OPEN **BEFORE** spawning background
                animation loops.  The loops check CurrentState == HUB_OPEN
                as their continuation condition — if we set the state after
                spawning, the loops exit on the first iteration.
            ]]
            CurrentState = State.HUB_OPEN

            -- Status dot pulse (only for supported games)
            if CurrentGame then
                task.spawn(function()
        ...
        local function AnimateMinimize()
            ...
            CreateToggleButton()  -- Sets state → MINIMIZED
        end
        ...
        local function AddButtonHover(btn, normal, hover)
            normal = normal or Theme.Surface
            hover  = hover  or Theme.SurfaceHover
        ...
        -- Close button
        hubConns:Connect(closeBtn.MouseButton1Click, Debounce(function()
            CreateRipple(closeBtn); AnimateClose()
        end, 1))
        hubConns:Connect(closeBtn.MouseEnter, function()
            QuickTween(closeBtn, { BackgroundTransparency = 0.15, TextColor3 = Theme.Error }, 0.15)
        end)
        hubConns:Connect(closeBtn.MouseLeave, function()
            QuickTween(closeBtn, { BackgroundTransparency = 0.5, TextColor3 = Theme.TextSecondary }, 0.15)
        end)

        -- Minimize button
        hubConns:Connect(minBtn.MouseButton1Click, Debounce(function()
            CreateRipple(minBtn); AnimateMinimize()
        end, 1))
        hubConns:Connect(minBtn.MouseEnter, function()
            QuickTween(minBtn, { BackgroundTransparency = 0.15, TextColor3 = Theme.Warning }, 0.15)
        end)
        hubConns:Connect(minBtn.MouseLeave, function()
        ...
        local onExecClick = Debounce(function()
            if ScriptLoaded then
                Notify("Info", "Script already loaded!", 3, Theme.Info)
                return
            end
            ...
            if IsAlive(statusDot) then
                QuickTween(statusDot, { BackgroundColor3 = Theme.Warning }, 0.2)
            end
            ...
            task.spawn(function()
                ...
                if success then
                    if IsAlive(execBtn) then
                        execBtn.Text = "✓  Loaded Successfully"
                    ...
                    if IsAlive(statusDot) then
                        QuickTween(statusDot, { BackgroundColor3 = Theme.Success }, 0.3)
                    end
                    Notify("Success", CurrentGame.Name .. " script loaded!", 4, Theme.Success)
                else
                    ExecutingLock = false
                    if IsAlive(execBtn) then
                        execBtn.Text = "Retry Execute"
                        QuickTween(execBtn, { BackgroundColor3 = Theme.Accent }, 0.3)
                    end
                    if IsAlive(statusLabel) then
                        statusLabel.Text = "Load failed — tap to retry"
                        QuickTween(statusLabel, { TextColor3 = Theme.Error }, 0.3)
                    end
                    if IsAlive(statusDot) then
                        QuickTween(statusDot, { BackgroundColor3 = Theme.Error }, 0.3)
                    end
                end
            end)
        end, 1.5)
    ...
    GlobalConns:Connect(UserInputService.InputBegan, function(input, processed)
        ...
        and input.KeyCode ~= Enum.KeyCode.F9 then return end

        if CurrentState == State.MINIMIZED then
            -- Restore from toggle
            CurrentState = State.CLOSING  -- guard against re-entry
            toggleConns:DisconnectAll()
            if IsAlive(ToggleButton) then
                QuickTween(ToggleButton, { BackgroundTransparency = 1, TextTransparency = 1 }, 0.2)
            end
        ...
        elseif CurrentState == State.HUB_OPEN then
            -- Quick-minimize via keyboard
            CurrentState = State.CLOSING
            pcall(function()
                if IsAlive(HubGui) then
                    local p = HubGui:FindFirstChild("Panel")
                    if p then
                        Settings.LastPosX = p.AbsolutePosition.X
                        Settings.LastPosY = p.AbsolutePosition.Y
                        SaveSettings()
                    end
                end
            end)
    ...
    CreateSplash(function()
        ...
        if not CurrentGame then
            Notify("Unsupported",
                "This game is not in the supported list.\n"
                .. SupportedGameCount .. " games available.",
                5, Theme.Warning)
        else
            Notify("Welcome",
