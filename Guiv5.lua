-- Solix Hub v4.2.0 | Production Loader | Full Key System | Xeno Compatible
...
local Config = {
    ...
    KeyFile      = "solixhub/savedkey.txt",
    GetKeyLink1  = "https://ads.luarmor.net/get_key?for=Solixhub_Free_KeySystem-OWlLHDMCHADk",
    GetKeyLink2  = "https://ads.luarmor.net/get_key?for=Solix_Free_Keysystems-pqJCGTqnTsng",
}
...
local Theme = {
    ...
    KeyInput      = Color3.fromRGB(30, 27, 36),
}

---------------------------------------------------------------------------
-- S6: Game Registry (with Keyless + XenoOK flags)
---------------------------------------------------------------------------
local GameRegistry = {
    ["3808223175"] = { Name = "Jujutsu Infinite",              LuarmorId = "4fe2dfc202115670b1813277df916ab2", Keyless = false, XenoOK = true  },
    ["994732206"]  = { Name = "Blox Fruits",                   LuarmorId = "e2718ddebf562c5c4080dfce26b09398", Keyless = false, XenoOK = true  },
    ["1650291138"] = { Name = "Demon Fall",                    LuarmorId = "9b64d07193c7c2aef970d57aeb286e70", Keyless = false, XenoOK = true  },
    ["1511883870"] = { Name = "Shindo Life",                   LuarmorId = "fefdf5088c44beb34ef52ed6b520507c", Keyless = false, XenoOK = true  },
    ["6035872082"] = { Name = "Rivals",                        LuarmorId = "3bb7969a9ecb9e317b0a24681327c2e2", Keyless = false, XenoOK = false },
    ["245662005"]  = { Name = "Jailbreak",                     LuarmorId = "21ad7f491e4658e9dc9529a60c887c6e", Keyless = true,  XenoOK = true  },
    ["7018190066"] = { Name = "Dead Rails",                    LuarmorId = "98f5c64a0a9ecca29517078597bbcbdb", Keyless = true,  XenoOK = true  },
    ["7074860883"] = { Name = "Arise Crossover",               LuarmorId = "0c8fdf9bb25a6a7071731b72a90e3c69", Keyless = false, XenoOK = false },
    ["7436755782"] = { Name = "Grow a Garden",                 LuarmorId = "e4ea33e9eaf0ae943d59ea98f2444ebe", Keyless = true,  XenoOK = true  },
    ["7326934954"] = { Name = "99 Nights in the Forest",       LuarmorId = "00e140acb477c5ecde501c1d448df6f9", Keyless = true,  XenoOK = true  },
    ["7671049560"] = { Name = "The Forge",                     LuarmorId = "c0b41e859f576fb70183206224d4a75f", Keyless = false, XenoOK = true  },
    ["6760085372"] = { Name = "Jujutsu: Zero",                 LuarmorId = "e380382a05647eabda3a9892f95952c6", Keyless = true,  XenoOK = true  },
    ["9266873836"] = { Name = "Anime Fighting Simulator",      LuarmorId = "3f9d315017ec895ded5c3350fd6e45a0", Keyless = true,  XenoOK = true  },
    ["3317771874"] = { Name = "Pet Simulator 99",              LuarmorId = "e95ef6f27596e636a7d706375c040de4", Keyless = true,  XenoOK = true  },
    ["9363735110"] = { Name = "Escape Tsunami For Brainrots!", LuarmorId = "4948419832e0bd4aa588e628c45b6f8d", Keyless = true,  XenoOK = true  },
    ["8144728961"] = { Name = "Abyss 67",                      LuarmorId = "50721a1cda76bf61b31ae6e7284a5ea3", Keyless = true,  XenoOK = true  },
    ["9509842868"] = { Name = "Gaarden Horizon",               LuarmorId = "cda910bd16c73785463fbe982d64994d", Keyless = true,  XenoOK = true  },
}
...
-- Derived flags
local NeedsKey = CurrentGame and not CurrentGame.Keyless or false

local IsLowExecutor = false
pcall(function()
    for _, n in ipairs({"Xeno", "Solara"}) do
        if string.find(ExecutorName, n) then
            IsLowExecutor = true
            pcall(function() Workspace:SetAttribute("low", true) end)
            break
        end
    end
end)
...
if IsLowExecutor and CurrentGame and not CurrentGame.Keyless and not CurrentGame.XenoOK then
...
---------------------------------------------------------------------------
-- S10: Luarmor API & Key System
---------------------------------------------------------------------------
local luarmor_api

if CurrentGame then
    pcall(function()
        luarmor_api = loadstring(game:HttpGet("https://sdkapi-public.luarmor.net/library.lua"))()
        luarmor_api.script_id = CurrentGame.LuarmorId
    end)
end
...
local LuarmorErrors = {
    ...
    KEY_HWID_LOCKED   = "Key is locked to a different device. Reset via Discord bot.",
    KEY_INCORRECT     = "Incorrect key. Please try again.",
    KEY_INVALID       = "Invalid key format.",
    SCRIPT_ID_INCORRECT = "Script not found on server.",
    SCRIPT_ID_INVALID   = "Script has been removed.",
    INVALID_EXECUTOR    = "Executor HWID not supported.",
    SECURITY_ERROR      = "Security check failed. Retry.",
    TIME_ERROR          = "System clock out of sync.",
...
-- Validate a key with Luarmor. Returns (success, message)
local function ValidateKey(key)
    if not luarmor_api then return false, "API not initialized" end
    local cleaned = key:gsub("%s", "")
    if not string.match(cleaned, "^[A-Za-z0-9]+$") or #cleaned ~= 32 then
        return false, "Invalid format (32 alphanumeric chars required)"
    end
    local ok, status = pcall(luarmor_api.check_key, cleaned)
    if not ok then return false, "Network error. Try again." end
    if type(status) ~= "table" then return false, "Unexpected response" end
    if status.code == "KEY_VALID" then
        script_key = cleaned -- Global used by Luarmor SDK
        KeyValidated = true
        pcall(function() writefile(Config.KeyFile, cleaned) end)
        local expiry = ""
        pcall(function()
            if status.data and status.data.auth_expire then
                expiry = " | Expires: " .. FormatTime(status.data.auth_expire - os.time())
            end
        end)
        return true, "Key validated" .. expiry
    end
    -- Invalid -- clear saved key
    pcall(function() if isfile(Config.KeyFile) then delfile(Config.KeyFile) end end)
    local msg = LuarmorErrors[status.code] or ("Error: " .. tostring(status.code))
    return false, msg
end

---------------------------------------------------------------------------
-- S11: Main Application
---------------------------------------------------------------------------
local function Main()
    ...
    local function Notify(title, desc, duration, color)
        ...
        local ab = Create("Frame", { Size = UDim2.new(0,3,1,0), BackgroundColor3 = color, BorderSizePixel = 0 }, frame)
        Create("UICorner", { CornerRadius = UDim.new(0,3) }, ab)
        ...
        Create("UICorner", { CornerRadius = UDim.new(0,2) }, tbg)
        local tf = Create("Frame", {
            Size = UDim2.new(1,0,1,0), BackgroundColor3 = color,
            BorderSizePixel = 0, BackgroundTransparency = 1,
        }, tbg)
        Create("UICorner", { CornerRadius = UDim.new(0,2) }, tf)
        ...
        task.defer(function()
            task.wait(0.08)
            QuickTween(tl, { TextTransparency = 0 }, 0.3)
            QuickTween(dl, { TextTransparency = 0 }, 0.3)
            task.wait(0.05)
            QuickTween(tbg, { BackgroundTransparency = 0 }, 0.25)
            QuickTween(tf,  { BackgroundTransparency = 0 }, 0.25)
            QuickTween(tf, { Size = UDim2.new(0,0,1,0) }, duration, Enum.EasingStyle.Linear)
        end)
        task.delay(duration, function()
            if not IsAlive(frame) then return end
            QuickTween(tl, { TextTransparency = 1 }, 0.25)
            QuickTween(dl, { TextTransparency = 1 }, 0.25)
            QuickTween(tbg, { BackgroundTransparency = 1 }, 0.25)
            QuickTween(tf,  { BackgroundTransparency = 1 }, 0.25)
    ...
    local function LoadGameScript()
        ...
        if not CurrentGame or not luarmor_api then return false end
        if ExecutorBlocked then
            Notify("Blocked", ExecutorName .. " not supported for " .. CurrentGame.Name, 5, Theme.Error)
            return false
        end
        if NeedsKey and not KeyValidated then
            Notify("Key Required", "Please validate your key first.", 4, Theme.Warning)
            return false
        end
        ...
        if ok then
            ...
            Notify("Load Error", "Script failed to load: " .. tostring(err), 6, Theme.Error)
            return false
        end
    end
    ...
    local function CreateSplash(onComplete)
        ...
        task.spawn(function()
            ...
            local ts = Create("UIScale", { Scale = 0.7 }, title)
            QuickTween(title, { TextTransparency = 0 }, 0.6, Enum.EasingStyle.Exponential)
            QuickTween(ts, { Scale = 1 }, 0.7, Enum.EasingStyle.Back)
            task.wait(0.2)
            task.spawn(function()
            ...
            task.wait(0.2)
            local stages = {
                { pct = 0.15, text = "Detecting executor..." },
                { pct = 0.30, text = "Executor: " .. ExecutorName },
                { pct = 0.50, text = "Scanning game registry..." },
                { pct = 0.65, text = CurrentGame and ("Found: " .. CurrentGame.Name) or "Game not in registry" },
                { pct = 0.80, text = NeedsKey and "Key required for this game" or "Keyless mode" },
                { pct = 1.00, text = "Ready" },
            }
            for _, s in ipairs(stages) do
                if IsAlive(status) then status.Text = s.text end
                QuickTween(barFill, { Size = UDim2.new(s.pct, 0, 1, 0) }, 0.4, Enum.EasingStyle.Quad)
                task.wait(0.5)
            end
            task.wait(0.35)
    ...
    CreateHub = function()
        ...
        local isKeyed = NeedsKey
        local panelW = IsMobile and 375 or 490
        local panelH
        if not CurrentGame then
            panelH = IsMobile and 350 or 330
        elseif isKeyed then
            panelH = IsMobile and 430 or 400
        else
            panelH = IsMobile and 350 or 330
        end

        -- Y-positions computed per layout
        local infoCardY = 62
        local infoCardH = 90

        local keyBoxY, keyBoxH, getKeyRowY, getKeyRowH
        local execBtnY, socialRowY

        if isKeyed then
            keyBoxY     = infoCardY + infoCardH + 10   -- 162
            keyBoxH     = 44
            getKeyRowY  = keyBoxY + keyBoxH + 8         -- 214
            getKeyRowH  = 36
            execBtnY    = getKeyRowY + getKeyRowH + 8   -- 258
            socialRowY  = execBtnY + 48 + 8             -- 314
        else
            execBtnY    = 164
            socialRowY  = 224
        end
        ...
        local infoCard = Create("Frame", {
            AnchorPoint = Vector2.new(0.5,0), Position = UDim2.new(0.5,0,0,infoCardY),
            Size = UDim2.new(0, panelW-36, 0, infoCardH), BackgroundColor3 = Theme.Surface,
        ...
        local detailText = string.format("ID: %s  |  %s  |  %d Games", CurrentGameId, ExecutorName, SupportedGameCount)
        ...
        if not CurrentGame then
            initialStatusColor = Theme.Error
            initialStatusText  = "Game not supported"
        elseif ExecutorBlocked then
            initialStatusColor = Theme.Error
            initialStatusText  = ExecutorName .. " not supported for this game"
        elseif isKeyed then
            if KeyValidated then
                initialStatusColor = Theme.Success
                initialStatusText  = "Key validated - ready"
            else
                initialStatusColor = Theme.Warning
                initialStatusText  = "Key required"
            end
        else
            initialStatusColor = Theme.Success
            initialStatusText  = "Ready to execute (keyless)"
        end

        local statusDot = Create("Frame", {
            AnchorPoint = Vector2.new(0,0.5), Position = UDim2.new(0,0,0,54),
            Size = UDim2.new(0,8,0,8), BackgroundColor3 = initialStatusColor,
        ...
        local statusLabel = Create("TextLabel", {
            ...
            BackgroundTransparency = 1, FontFace = FontMedium, Text = initialStatusText,
            TextColor3 = initialStatusColor, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left,
            TextTransparency = 1, ZIndex = 6,
        }, infoCard)

        -- ============================
        -- KEY INPUT (only for keyed games)
        -- ============================
        local keyContainer, keyTextBox
        local getKeyRow, getKeyBtn1, getKeyBtn2, getKeyStroke1, getKeyStroke2

        if isKeyed then
            -- Key input container
            keyContainer = Create("Frame", {
                AnchorPoint = Vector2.new(0.5,0), Position = UDim2.new(0.5,0,0,keyBoxY),
                Size = UDim2.new(0, panelW-36, 0, keyBoxH), BackgroundColor3 = Theme.KeyInput,
                BorderSizePixel = 0, BackgroundTransparency = 1, ZIndex = 5,
            }, panel)
            Create("UICorner", { CornerRadius = UDim.new(0,8) }, keyContainer)
            local keyContainerStroke = Create("UIStroke", {
                Color = Theme.Border, Thickness = 1, Transparency = 1,
                ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
            }, keyContainer)

            keyTextBox = Create("TextBox", {
                Size = UDim2.new(1,-24,1,0), Position = UDim2.new(0,12,0,0),
                BackgroundTransparency = 1, FontFace = FontMedium,
                PlaceholderText = "Paste your key here...",
                PlaceholderColor3 = Theme.TextTertiary,
                Text = "", TextColor3 = Theme.Text, TextSize = 14,
                ClearTextOnFocus = false, TextTransparency = 1,
            ...
            pcall(function()
                if isfile(Config.KeyFile) then
                    ...
                    if saved and #saved > 0 then
                        keyTextBox.Text = saved
                    end
                end
            end)

            -- Get Key buttons row
            getKeyRow = Create("Frame", {
                AnchorPoint = Vector2.new(0.5,0), Position = UDim2.new(0.5,0,0,getKeyRowY),
                Size = UDim2.new(0, panelW-36, 0, getKeyRowH), BackgroundTransparency = 1, ZIndex = 5,
            }, panel)

            local gkHalfW = math.floor((panelW - 36 - 10) / 2)

            getKeyBtn1 = Create("TextButton", {
                Position = UDim2.new(0,0,0,0), Size = UDim2.new(0,gkHalfW,1,0),
                BackgroundColor3 = Theme.Surface, FontFace = FontMedium,
                Text = "Get Key (Ad Link)", TextColor3 = Theme.TextSecondary, TextSize = 12,
                BorderSizePixel = 0, AutoButtonColor = false,
                BackgroundTransparency = 1, TextTransparency = 1, ClipsDescendants = true, ZIndex = 5,
            }, getKeyRow)
            Create("UICorner", { CornerRadius = UDim.new(0,8) }, getKeyBtn1)
            getKeyStroke1 = Create("UIStroke", { Color = Theme.Border, Thickness = 1, Transparency = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }, getKeyBtn1)

            getKeyBtn2 = Create("TextButton", {
                AnchorPoint = Vector2.new(1,0), Position = UDim2.new(1,0,0,0), Size = UDim2.new(0,gkHalfW,1,0),
                BackgroundColor3 = Theme.Surface, FontFace = FontMedium,
                Text = "Get Key (Shrink)", TextColor3 = Theme.TextSecondary, TextSize = 12,
                BorderSizePixel = 0, AutoButtonColor = false,
                BackgroundTransparency = 1, TextTransparency = 1, ClipsDescendants = true, ZIndex = 5,
            }, getKeyRow)
            Create("UICorner", { CornerRadius = UDim.new(0,8) }, getKeyBtn2)
            getKeyStroke2 = Create("UIStroke", { Color = Theme.Border, Thickness = 1, Transparency = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }, getKeyBtn2)
        end
        ...
        local execBtnColor
        local execBtnText
        if not CurrentGame or ExecutorBlocked then
            execBtnColor = Color3.fromRGB(55,52,62)
            execBtnText = ExecutorBlocked and "Executor Blocked" or "Not Supported"
        elseif isKeyed and not KeyValidated then
            execBtnColor = Theme.SurfaceLight
            execBtnText = "Validate & Execute"
        else
            execBtnColor = Theme.Accent
            execBtnText = "Execute Script"
        end

        local execBtn = Create("TextButton", {
            AnchorPoint = Vector2.new(0.5,0), Position = UDim2.new(0.5,0,0,execBtnY),
        ...
        if CurrentGame and not ExecutorBlocked and (not isKeyed or KeyValidated) then
        ...
        local socialRow = Create("Frame", {
            AnchorPoint = Vector2.new(0.5,0), Position = UDim2.new(0.5,0,0,socialRowY),
            Size = UDim2.new(0, panelW-36, 0, isKeyed and 36 or 40), BackgroundTransparency = 1, ZIndex = 5,
        }, panel)
        ...
        local socialBtn2Text = isKeyed and "Buy Lifetime Key" or "Website"
        local websiteBtn, websiteStroke = MakeSocialBtn(socialBtn2Text, 1, 1)

        -- Footer
        local footerText
        if isKeyed then
            footerText = Config.Version .. "  |  Key Required  |  " .. SupportedGameCount .. " Games"
        else
            footerText = Config.Version .. "  |  Keyless  |  " .. SupportedGameCount .. " Games Supported"
        end
        local footer = Create("TextLabel", {
            AnchorPoint = Vector2.new(0.5,1), Position = UDim2.new(0.5,0,1,-10),
            Size = UDim2.new(1,-36,0,18), BackgroundTransparency = 1,
            FontFace = FontMedium, Text = footerText,
        ...
        -- Add keyed-only elements
        if isKeyed then
            table.insert(fadeElements, { obj=keyContainer, prop="BackgroundTransparency", to=0 })
            table.insert(fadeElements, { obj=keyTextBox,   prop="TextTransparency",       to=0 })
            table.insert(fadeElements, { obj=getKeyBtn1,   prop="BackgroundTransparency", to=0 })
            table.insert(fadeElements, { obj=getKeyBtn1,   prop="TextTransparency",       to=0 })
            table.insert(fadeElements, { obj=getKeyBtn2,   prop="BackgroundTransparency", to=0 })
            table.insert(fadeElements, { obj=getKeyBtn2,   prop="TextTransparency",       to=0 })

            local kcStroke = keyContainer:FindFirstChildOfClass("UIStroke")
            if kcStroke then table.insert(allStrokes, kcStroke) end
            table.insert(allStrokes, getKeyStroke1)
            table.insert(allStrokes, getKeyStroke2)
        end
        ...
        local function UpdateStatus(text, color)
            if IsAlive(statusLabel) then
                ...
                QuickTween(statusLabel, { TextColor3 = color }, 0.3)
            end
            if IsAlive(statusDot) then
                QuickTween(statusDot, { BackgroundColor3 = color }, 0.3)
            end
        end
        ...
        local function AnimateOpen()
            ...
            -- Status dot pulse
            if CurrentGame and not ExecutorBlocked then
                task.spawn(function()
        ...
        -- Key TextBox FocusLost (auto-validate)
        if isKeyed and keyTextBox then
            hubConns:Connect(keyTextBox.FocusLost, function(enterPressed)
                if not keyTextBox or keyTextBox.Text == "" then return end
                if KeyValidated then return end -- Already validated

                UpdateStatus("Validating key...", Theme.Warning)
                local stopAnim = AnimateLoadingText(statusLabel, "Validating")

                task.spawn(function()
                    local valid, msg = ValidateKey(keyTextBox.Text)
                    stopAnim()

                    if valid then
                        UpdateStatus("Key valid - ready to execute", Theme.Success)
                        Notify("Key Valid", msg, 4, Theme.Success)
                        -- Update execute button
                        if IsAlive(execBtn) then
                            execBtn.Text = "Execute Script"
                            QuickTween(execBtn, { BackgroundColor3 = Theme.Accent }, 0.3)
                        end
                    else
                        UpdateStatus("Key invalid", Theme.Error)
                        Notify("Invalid Key", msg, 5, Theme.Error)
                        keyTextBox.Text = ""
                    end
                end)
            end)
        ...
        local onExecClick = Debounce(function()
            ...
            if not CurrentGame then Notify("Error", "Game not supported.", 4, Theme.Error); return end
            if ExecutorBlocked then Notify("Blocked", ExecutorName .. " is blocked for " .. CurrentGame.Name .. ".", 5, Theme.Error); return end
            if ExecutingLock then return end

            -- For keyed games: validate first if not yet validated
            if isKeyed and not KeyValidated then
                local keyText = keyTextBox and keyTextBox.Text or ""
                if keyText == "" then
                    Notify("Key Required", "Enter your key above or use Get Key buttons.", 4, Theme.Warning)
                    return
                end
                -- Validate inline
                UpdateStatus("Validating key...", Theme.Warning)
                local stopAnim1 = AnimateLoadingText(statusLabel, "Validating")
                local valid, msg = ValidateKey(keyText)
                stopAnim1()
                if not valid then
                    UpdateStatus("Key invalid", Theme.Error)
                    Notify("Invalid Key", msg, 5, Theme.Error)
                    if keyTextBox then keyTextBox.Text = "" end
                    return
                end
                UpdateStatus("Key valid", Theme.Success)
                Notify("Key Valid", msg, 3, Theme.Success)
                task.wait(0.3) -- Brief pause before executing
            end
            ...
            UpdateStatus("Loading script...", Theme.Warning)
            ...
            task.spawn(function()
                ...
                if success then
                    ...
                    UpdateStatus("Script loaded", Theme.Success)
                    Notify("Success", CurrentGame.Name .. " loaded!", 4, Theme.Success)
                else
                    ExecutingLock = false
                    if IsAlive(execBtn) then execBtn.Text = isKeyed and "Validate & Execute" or "Retry Execute"; QuickTween(execBtn, { BackgroundColor3 = isKeyed and Theme.SurfaceLight or Theme.Accent }, 0.3) end
                    UpdateStatus("Load failed - try again", Theme.Error)
                end
            end)
        end, 1.5)
        ...
        if CurrentGame and not ExecutorBlocked and (not isKeyed or KeyValidated) then
            AddButtonHover(execBtn, Theme.Accent, Theme.AccentLight)
        end

        -- Get Key buttons (keyed only)
        if isKeyed then
            hubConns:Connect(getKeyBtn1.MouseButton1Click, Debounce(function()
                CreateRipple(getKeyBtn1); pcall(setclipboard, Config.GetKeyLink1)
                Notify("Copied", "Key link copied to clipboard.", 3, Theme.Info)
            end, 1))
            AddButtonHover(getKeyBtn1)

            hubConns:Connect(getKeyBtn2.MouseButton1Click, Debounce(function()
                CreateRipple(getKeyBtn2); pcall(setclipboard, Config.GetKeyLink2)
                Notify("Copied", "Key link copied to clipboard.", 3, Theme.Info)
            end, 1))
            AddButtonHover(getKeyBtn2)
        end

        -- Discord
        hubConns:Connect(discordBtn.MouseButton1Click, Debounce(function()
            CreateRipple(discordBtn); pcall(setclipboard, Config.Discord)
            Notify("Copied", "Discord invite copied.", 3, Theme.Info)
        end, 1))
        ...
        hubConns:Connect(websiteBtn.MouseButton1Click, Debounce(function()
            ...
            Notify("Copied", isKeyed and "Shop link copied. $15 for lifetime key." or "Website copied.", 3, Theme.Info)
        end, 1))
        ...
        -- Auto-validate saved key after open
        if isKeyed and not KeyValidated then
            task.spawn(function()
                task.wait(0.8) -- Let hub finish opening
                local savedKey = ""
                pcall(function()
                    if isfile(Config.KeyFile) then
                        savedKey = readfile(Config.KeyFile)
                    end
                end)
                if savedKey ~= "" and #savedKey > 0 then
                    Notify("Info", "Found saved key, validating...", 3, Theme.Info)
                    UpdateStatus("Checking saved key...", Theme.Warning)
                    local valid, msg = ValidateKey(savedKey)
                    if valid then
                        UpdateStatus("Key valid - ready", Theme.Success)
                        Notify("Key Valid", msg, 4, Theme.Success)
                        if IsAlive(execBtn) then
                            execBtn.Text = "Execute Script"
                            QuickTween(execBtn, { BackgroundColor3 = Theme.Accent }, 0.3)
                        end
                        if IsAlive(keyTextBox) then keyTextBox.Text = savedKey end
                        -- Auto-execute if configured
                        if Config.AutoExecute and not ScriptLoaded then
                            task.wait(1)
                            if not ScriptLoaded and IsAlive(execBtn) then
                                onExecClick()
                            end
                        end
                    else
                        UpdateStatus("Saved key invalid", Theme.Error)
                        Notify("Saved Key Invalid", msg, 4, Theme.Error)
                        if IsAlive(keyTextBox) then keyTextBox.Text = "" end
                    end
                end
            end)
        end

        -- Auto-execute for keyless games
        if not isKeyed and Config.AutoExecute and CurrentGame and not ScriptLoaded and not ExecutorBlocked then
    ...
    CreateSplash(function()
        ...
        task.wait(0.6)
        if not CurrentGame then
            Notify("Unsupported", "Game not in registry. " .. SupportedGameCount .. " games supported.", 5, Theme.Warning)
        elseif ExecutorBlocked then
            Notify("Executor Blocked", ExecutorName .. " is not supported for " .. CurrentGame.Name .. ". Try a different executor.", 6, Theme.Error)
        elseif isKeyed then
            Notify("Welcome", CurrentGame.Name .. " requires a key. Get one using the buttons below.", 5, Theme.Warning)
        else
            Notify("Welcome", "Playing " .. CurrentGame.Name .. " | Keyless", 4, Theme.Accent)
        end
    end)
end
...
if not ok then
    warn("[Solix Hub] Fatal: " .. tostring(err))
end
